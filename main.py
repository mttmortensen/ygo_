from ac_api_call import ygo_ac_call # Import the new functions
from db_utils import create_connection, check_table_exists, count_rows_in_table, insert_data, fetch_all_card_data # Import the new functions
from pandas_data_handle import convert_to_dataframe, fill_missing_values, format_numeric_columns, flatten_deck_data  # Import the new functions
...
# Flatten deck data
flattened_deck_data = [flatten_deck_data(deck, df) for deck in deck_data]

# Display the first 10 rows of the new dataset
for i, deck_df in enumerate(flattened_deck_data):
    print(f"First 10 rows of deck {i + 1}:")
